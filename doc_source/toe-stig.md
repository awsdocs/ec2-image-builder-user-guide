...
Windows STIG components are designed for standalone servers and they apply Local Group Policy\. STIG\-compliant components install InstallRoot on Windows AMIs from the Department of Defense \(DoD\) to install and update the DoD certificates\. They also remove unnecessary certificates to maintain STIG compliance\. Currently, STIG baselines are supported for the following versions of Windows Server: 2012 R2, 2016, and 2019\.

### STIG\-Build\-Windows\-Low version 1\.5\.0<a name="ib-windows-stig-low"></a>
...
For a complete list of Windows STIGs, see the [STIGs Document Library](https://public.cyber.mil/stigs/downloads/?_dl_facet_stigs=windows)\. For information about STIG viewing tools and guidelines, see [SRG / STIG Tools](https://public.cyber.mil/stigs/srg-stig-tools/)\.
...
+ **Windows Server 2012 R2 STIG Version 3 Release 3**
...
### STIG\-Build\-Windows\-Medium version 1\.5\.0<a name="ib-windows-stig-medium"></a>
...
### STIG\-Build\-Windows\-High version 1\.5\.0<a name="ib-windows-stig-high"></a>
...
The Security Content Automation Protocol \(SCAP\) is a set of standards that are used to identify application security vulnerabilities for compliance\. The SCAP Compliance Checker \(SCC\) is a SCAP\-validated scanning tool, released by the Naval Information Warfare Center \(NIWC\) Atlantic\. For more information, see [Security Content Automation Protocol \(SCAP\) Compliance Checker \(SCC\)](https://www.niwcatlantic.navy.mil/scap/) on the *NIWC Atlantic* website\.

The Image Builder `scap-checker-windows` and `scap-checker-linux` components download and install the SCC scanner on the pipeline build and test instances\. When the scanner runs, it performs authenticated configuration scanning using DISA SCAP Benchmarks, and provides a report that includes the following information, which is also recorded in your application logs:
+ STIG settings that have been applied to the instance\.
+ An overall compliance score for the instance\.

We recommend that you run SCAP validation as the final step in your build process, to ensure that you are reporting accurate compliance validation results\.

**Note**  
You can review the reports using one of the [STIG Viewing Tools](https://public.cyber.mil/stigs/srg-stig-tools/), available online via the DoD Cyber Exchange\.

The following sections describe the benchmarks that are included in the SCAP validation components\.

### scap\-checker\-windows version 2021\.04\.0<a name="scap-component-windows"></a>

The `scap-checker-windows` component runs on the Image Builder pipeline's build and test instances\. Image Builder logs both the report and the score that the SCC application produces\.
...
**Note**  
Image Builder currently supports SCAP compliance validation for Windows Server 2012 R2, 2016, and 2019\.
...
The `scap-checker-linux` component runs on the Image Builder pipeline's build and test instances\. Image Builder logs both the report and the score that the SCC application produces\.
...
**Note**  
Image Builder currently supports SCAP compliance validation for RHEL 7/8 and Ubuntu 18\. The SCC application currently supports the x86 architecture for validation\.
